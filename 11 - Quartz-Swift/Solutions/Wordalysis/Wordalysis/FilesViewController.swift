...
class FilesViewController: UITableViewController {
    ...
    var processedCount: Int {
        let total = counters.map({$0.currentState.processedCount}).reduce(0,+)
        return total
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        vc.wordCounter = counter
        print("vc word counter is \(vc.wordCounter)")
    }
            ...
            try textFinder.withTexts { [weak self] (result) in
                ...
                case .success(let texts):
                    ...
                    strongSelf.progressGroup?.notify(queue: DispatchQueue.main) {
                        strongSelf.navigationItem.title = "\(strongSelf.processedCount) Words"
    ...
    func update() {
        navigationItem.title = "\(processedCount) words"
        
        guard let visibleIndexPaths = tableView.indexPathsForVisibleRows else { return }
        for indexPath in visibleIndexPaths {
            guard let cell = tableView.cellForRow(at: indexPath) else { continue }
            let counter = counters[indexPath.row]
            let count = counter.currentState.processedCount
    ...
    func presentCompletionAlert() {
        let alert = UIAlertController(title: "Analysis Complete",
                                      message: "\(processedCount) words found across \(counters.count) files",
